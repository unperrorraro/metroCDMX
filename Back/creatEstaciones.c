...
int wich_attribute(char *pal) {
  if (strcmp(pal, "estacion") == 0)
    return ESTACION;
  if (strcmp(pal, "linea") == 0)
    return LINEA;
  if (strcmp(pal, "transbordo") == 0)
    return TRANSBORDO;
  if (strcmp(pal, "adyacentes") == 0)
    return ADYACENTES;
  return DESCONOCIDO;
}
...
void formatear_estacion(estacion *est) {
  ...
  printf("Nombre : %s| \n Puntero : %p \n Líneas : [ %d, %d, %d ] \n "
         "transbordo : %d \n numero adyacentes:  %d\n  adyacentes : [",
         est->nombre, est, est->lineas[0], est->lineas[1], est->lineas[2],
         est->transbordo, est->num_ady);
...
estacion **crear_estaciones(char *in) {
  ...
  estacion **estaciones_array = malloc(NUM_EST * sizeof(estacion *));
  for (int i = 0; i < NUM_EST; i++) {
  ...
  while ((c = getc(fptr)) != EOF) {
    ...
    while ((c = getc(fptr)) != EOF) {
      ...
      case TRANSBORDO: {
        ...
        if (c == 'F') {
          ...
          fprintf(stderr, "ERROR: transbordo debe ser true o false\n");
          exit(EXIT_FAILURE);
        }

        if (c = getc(fptr) != '"') {
          fprintf(stderr, "ERROR: error al finalizar string\n");
          exit(EXIT_FAILURE);
        }
        break;
      }
        ...
        while ((c = getc(fptr)) != EOF) {
          ...
          if (c == ']') {
            ...
            if (!est->adyacentes) {
              fprintf(
                  stderr,
                  "ERROR: no se pudo reservar memoria para adyacentes de %s\n",
                  est->nombre);
              exit(EXIT_FAILURE);
            }
            break;
    ...
    if (c == ']') {
      ...
      // formatear_estacion(estaciones_array[j]);}
      break;
    }
  }
